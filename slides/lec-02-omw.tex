...
should \lex{water} link to \lex{水} or do we need a non lexicalized entry \lex{水}-or-\lex{湯}?  
...
\item \txx{temperature} is linked by \lnk{attribute} (属性); tree is \lnk{hyponym} 
...
\end{frame}
\begin{frame}[allowframebreaks]{Other Examples --- ロングテール ``long tail'')}
  \begin{exe}
  \ex \label{wn:soba}
\begin{avm} \small
...
\[ \wnida{\wnid{80002377-n} (castle construction)}
  lemmas:jpn &\wnbox{築城 \jpn{chikujou}}  \\
def:jpn &\wnbox{城の\,建設} \\ 
...
\[ \wnida{\wnid{90000315-n} (hajjah)}
  lemmas:jpn &\wnbox{ハジャ} \\
lemmas:eng &\wnbox{hajjah} \\
  def:jpn &\wnbox{メッカへの\,巡礼を\,行った\,女性} \\ 
...
\[ \wnida{\wnid{80001731-n} (exchange student)}
  lemmas:jpn &\wnbox{留学生} \\
lemmas:eng &\wnbox{exchange student} \\
  def:jpn &\wnbox{海外で\,勉強する\,学生} \\ 
...
\[ \wnida{\wnid{80000338-n} (Shunto)}
  ...
  def:jpn &\wnbox{毎年労働組合が、賃金引き\,上げなどの} \\
   &\wnbox{\ \ \ \  要求を\,掲げて\,行う\,全国的な\,闘争} \\ 
