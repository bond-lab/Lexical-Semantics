...
\begin{itemize}
  ...
  \item \textbf{Variant forms}
    \begin{itemize}
    \item spelling variants (\eng{colour/color}), scripts (\jpn{犬 / いぬ}), casing/diacritics policy,
...
%%% FIXME add pronunciation example
%%% FIXME add wn code
...
\begin{tikzpicture}[node distance=8mm, scale=0.72, transform shape]
  % Noun side
  \node[synset] (syn_n) {Synset (N)\\\textbf{LIGHT\_N}\\\tiny “illumination”};
  \node[sense, left=12mm of syn_n, yshift=7mm] (s_n1) {Sense\\light\textsuperscript{n1}\\\tiny “illumination”};
  \node[sense, left=12mm of syn_n, yshift=-7mm] (s_n2) {Sense\\illumination\textsuperscript{n1}};
  \node[word, left=18mm of s_n1] (w_light_n) {Word\\\textbf{light}};
  \node[word, left=8mm of s_n2] (w_illum) {Word\\\textbf{illumination}};

  % Adjective side
  \node[synset, right=24mm of syn_n] (syn_a) {Synset (ADJ)\\\textbf{LIGHT\_A}\\\tiny “not heavy / bright”};
  \node[sense, right=12mm of syn_a, yshift=7mm] (s_a1) {Sense\\light\textsuperscript{a1}};
  \node[sense, right=12mm of syn_a, yshift=-7mm] (s_a2) {Sense\\bright\textsuperscript{a1}};
  \node[word, right=18mm of s_a1] (w_light_a) {Word\\\textbf{light}};
  \node[word, right=18mm of s_a2] (w_bright) {Word\\\textbf{bright}};
...
\begin{tikzpicture}[node distance=10mm, scale=0.80, transform shape]
  \node[synset] (dog) {synset\\\textbf{DOG}};
  \node[synset, above=12mm of dog] (canine) {synset\\\textbf{CANINE}};
  \node[synset, above=12mm of canine] (animal) {synset\\\textbf{ANIMAL}};

  \node[synset, right=28mm of dog] (cat) {synset\\\textbf{CAT}};
  \node[synset, above=12mm of cat] (feline) {synset\\\textbf{FELINE}};


  \node[synset, below=10mm of dog] (bark) {synset\\\textbf{BARK\_N}};
  \node[synset, left=10mm of bark] (flag) {synset\\\textbf{FLAG\_N}};
  \node[synset, right=20mm of bark] (barkv) {synset\\\textbf{BARK\_V}};
...
\begin{tikzpicture}[node distance=8mm, scale=0.75, transform shape, xshift=-6mm]
  \node[synset] (entity) {synset\\\textbf{ENTITY}};
  \node[synset, below left=5mm and 14mm of entity] (physical) {synset\\\textbf{PHYSICAL\_ENTITY}};
  \node[synset, below right=5mm and 14mm of entity] (abstract) {synset\\\textbf{ABSTRACT\_ENTITY}};

  \node[synset, below=5mm of physical] (organism) {synset\\\textbf{ORGANISM}};
  \node[synset, below left=5mm and 12mm of organism] (animal) {synset\\\textbf{ANIMAL}};
  \node[synset, below right=5mm and 12mm of organism] (plant) {synset\\\textbf{PLANT}};

  \node[synset, below=5mm of animal] (canine) {synset\\\textbf{CANINE}};
  \node[synset, below=5mm of canine] (dog) {synset\\\textbf{DOG}};
...
\begin{tikzpicture}[node distance=10mm]
  % Language synsets
  \node[synset] (en_dog) {EN synset\\\textbf{DOG}};
  \node[synset, below=10mm of en_dog] (de_hund) {DE synset\\\textbf{HUND}};
  \node[synset, below=10mm of de_hund] (ja_inu) {JA synset\\\textbf{犬} (inu)};

  % ILI
  \node[ili, right=30mm of de_hund] (ili_dog) {ILI\\\textbf{i46360}};
